...
func _build_retro_ui() -> void:
	...
	var title_lbl := _label("✨ 공기정화 마법봉 충전 - 환경 요소 분류 게임", Vector2(40, 35), Vector2(1200, 40), 28, Color("FFFFFF"))
	title_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(title_lbl)

	# 안내 텍스트
	var info_lbl := _label("공기를 오염시키는 원인은 [⬅️ 왼쪽], 공기를 정화하는 요소는 [➡️ 오른쪽]으로 분류하여 마법봉을 충전하세요!", Vector2(0, 105), Vector2(1280, 30), 20, Color("555555"))
	info_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(info_lbl)

	# 마법봉 충전 게이지 영역
	var gauge_box := ColorRect.new()
	gauge_box.color = Color("EFE8D8")
	...
	card_label = _label("🚗 자동차 매연", Vector2(0, 50), Vector2(500, 60), 36, Color("333333"))
	card_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	card_panel.add_child(card_label)

	card_hint_label = _label("이 요소는 공기를 오염시킬까요, 깨끗하게 할까요?", Vector2(20, 140), Vector2(460, 60), 20, Color("666666"))
	...
	# 피드백 텍스트
	feedback_label = _label("키보드 [← / A] 또는 [→ / D] 키를 누르거나 아래 버튼을 클릭하세요!", Vector2(140, 480), Vector2(1000, 40), 22, Color("333333"))
	feedback_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(feedback_label)

	# 하단 좌우 분류 버튼
	btn_left = _create_retro_button("⬅️ 오염 원인\n[왼쪽 / A]", Vector2(180, 540), Vector2(380, 90), Color("D94141"), Color("FFFFFF"))
	btn_left.pressed.connect(func(): _classify_card(CardType.POLLUTION))
	add_child(btn_left)

	btn_right = _create_retro_button("➡️ 정화 요소\n[오른쪽 / D]", Vector2(720, 540), Vector2(380, 90), Color("4CAF50"), Color("FFFFFF"))
	...
	var win_title := _label("🎉 공기정화 마법봉 100% 충전 완료! ✨", Vector2(0, 40), Vector2(700, 50), 32, Color("DA863E"))
	win_title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	win_panel.add_child(win_title)

	var win_desc := _label("완벽합니다! 여러분의 환경 지혜로 맑고 강력한\n'공기정화 마법봉'이 완성되었습니다!\n\n이제 숨쉬기 힘들어하는 씨앗에게 돌아가\n까만 매연 구름을 모두 깨끗이 정화해 줍시다!", Vector2(50, 110), Vector2(600, 120), 22, Color("333333"))
	...
	btn_return = _create_retro_button("🌱 텃밭으로 돌아가서 매연 정화하기!", Vector2(150, 290), Vector2(400, 60), Color("DA863E"), Color("FFFFFF"))
...
func _show_next_card() -> void:
	...
	card_hint_label.add_theme_color_override("font_color", Color("666666"))
...
func _classify_card(chosen_type: int) -> void:
	...
	if chosen_type == card.type:
		...
		card_hint_label.add_theme_color_override("font_color", Color("DA863E"))
...
func _on_game_cleared() -> void:
	...
	feedback_label.add_theme_color_override("font_color", Color("DA863E"))
	win_panel.show()
