...
func _build_ui() -> void:
	...
	# 상호작용(말하기) 버튼 (20% 축소: 160 -> 130, X축 우측 50px, 최종적으로 Y=540으로 세밀 조정)
	_add_touch_button(layer, Vector2(1120, 540), &"interact", "말하기\n(Space)", Vector2(130, 130))
	...
	if GameState.current_quest == &"water":
		match water_phase:
			...
			WaterPhase.GO_TO_GRANDMA:
				_show_dialogue(
					"마을 할머니",
					"호호, 씨앗에게 물이 필요한 거로구나!\n그런데 어쩌지? 남은 물뿌리개를 마을 어딘가에서 잃어버렸단다.\n마을 구석구석을 뒤져 물뿌리개 3개를 모두 찾아와 주겠니?",
					"물뿌리개 3개 찾기!",
			...
			WaterPhase.RETURN_TO_GRANDMA:
				_show_dialogue(
					"마을 할머니",
					"오! 잃어버린 물뿌리개 3개를 모두 찾아주었구나, 정말 고맙다!\n약속대로 물뿌리개를 너에게 선물로 주마.\n이제 목말라하는 씨앗에게 시원한 물을 주렴!",
					"고맙습니다! 씨앗에게 갈게요.",
					_accept_got_can,
					"",
					Callable()
...
func _add_touch_button(layer: CanvasLayer, button_position: Vector2, action: StringName, caption: String, button_size := Vector2(110, 110)) -> void:
	...
	var shape := CircleShape2D.new()
	shape.radius = button_size.x / 2.0
	touch.shape = shape
	touch.action = action
	...
	style.corner_radius_top_left = int(button_size.x / 2.0)
	style.corner_radius_top_right = int(button_size.x / 2.0)
	style.corner_radius_bottom_left = int(button_size.x / 2.0)
	style.corner_radius_bottom_right = int(button_size.x / 2.0)
	bg.add_theme_stylebox_override("panel", style)
	bg.mouse_filter = Control.MOUSE_FILTER_IGNORE
	bg.modulate = Color(1.0, 1.0, 1.0, 0.5) # 약간 투명한 하얀색
	touch.add_child(bg)
