...
function Process-Char2ToChar1($inPath, $outPath) {
    Write-Host "Processing $inPath -> $outPath with BBox alignment..."
    $src = [System.Drawing.Bitmap]::FromFile($inPath)
    
    # 4열 4행, 1칸당 가로 200, 세로 280 => 총 가로 800, 세로 1120
    $w = 200; $h = 280
    ...
    for ($r=0; $r -lt 4; $r++) {
        for ($c=0; $c -lt 4; $c++) {
            ...
            # 1. 이 프레임 주변(가로 300, 세로 340)에서 체크보드 제외 픽셀들의 바운딩 박스 조사
            ...
            if ($min_x -le $max_x) {
                ...
                # 2. 발바닥 정렬(하단 15px 위에 발바닥 배치) & 수평 중앙 정렬
                ...
                Write-Host "Row $r Col $c : Aligned char ($char_w x $char_h) at cell offset ($dest_cell_x, $dest_cell_y)"
            }
        }
    }
    $g.Dispose()
